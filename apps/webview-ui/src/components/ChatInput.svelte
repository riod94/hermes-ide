    ...
    <div class="attachment-chips px-3 pt-2">
      {#each $attachments as attachment, i}
        <div class="attachment-chip">
          <span class="chip-icon">{attachment.type === 'file' ? '📄' : attachment.type === 'folder' ? '📁' : attachment.type === 'terminal' ? '⬛' : attachment.type === 'url' ? '🔗' : attachment.type === 'image' ? '🖼️' : attachment.type === 'selection' ? '✂️' : '📏'}</span>
