...
const NGINX_SITES_PATH = "/etc/nginx/sites-available/ide.dev.nusa.work";
const NGINX_ENABLED_PATH = "/etc/nginx/sites-enabled/ide.dev.nusa.work";

// SSL & domain config
const IDE_DOMAIN = process.env.IDE_DOMAIN || "ide.dev.nusa.work";          // parent domain
const AUTH_PORTAL_DOMAIN = process.env.AUTH_PORTAL_DOMAIN || "portal.ide.dev.nusa.work"; // Auth Portal subdomain
const SSL_CERT = process.env.SSL_CERT || "/etc/letsencrypt/live/ide.dev.nusa.work/fullchain.pem";
const SSL_KEY = process.env.SSL_KEY || "/etc/letsencrypt/live/ide.dev.nusa.work/privkey.pem";
const AUTH_PORTAL_PORT = process.env.AUTH_PORTAL_PORT ? parseInt(process.env.AUTH_PORTAL_PORT, 10) : 51000;

const CODE_SERVER_IMAGE = "lscr.io/linuxserver/code-server:latest";
const HERMES_PROFILES_DIR = "/home/ade/.hermes/profiles";
const HERMES_HOST_IP = process.env.HERMES_HOST_IP || "103.196.116.158";
...
export function generateDockerCompose(store: ProfileStore): string {
  ...
  for (const profile of store.profiles) {
    ...
    environment:
      ...
      - HERMES_API_URL=http://${HERMES_HOST_IP}:${apiPort}/v1
      - HERMES_API_KEY=${hermesApiKey}
      - HERMES_UPSTREAM_URL=http://${HERMES_HOST_IP}:20128/v1
 ...
 * - ide.dev.nusa.work → Auth Portal (port 51000)
 * - {name}.ide.dev.nusa.work → code-server container (per profile port)
...
# ──────────────────────────────────────────────
# Code-server profiles: {name}.ide.dev.nusa.work
# ──────────────────────────────────────────────
${profileServerBlocks}
`;
}
